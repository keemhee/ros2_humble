...
def process_frame(frame):
    ...
    if len(contours) >= 1:
        ...
        if M["m00"] != 0:
            ...
            if diff < -20:
                return "LEFT", yellow_binary_image
            elif diff > 20:
                return "RIGHT", yellow_binary_image
            else:
                return "FORWARD", yellow_binary_image
            """
            if cx < width / 3:
                return "LEFT"
            elif cx > width / 3 * 2:
            ...
            return "STOP", yellow_binary_image
    return None, yellow_binary_image
def camera_thread():
    ...
    while True:
        ...
        if not ret:
            print("Camera error")
            break
        result, result_frame = process_frame(frame)
        if result:
            yellow_direction = result
        cv2.imshow("Line Tracer", result_frame)
    ...
    while True:
        ...
        if yellow_direction and run_flag:
            if yellow_direction == "LEFT":
                print("Turning LEFT")
                agv.counterclockwise_rotation(30)  # 각도 조정
                time.sleep(0.5)  # 회전 시간
                #agv.stop()
            elif yellow_direction == "RIGHT":
                print("Turning RIGHT")
                agv.clockwise_rotation(30)  # 각도 조정
                time.sleep(0.5)  # 회전 시간
                #agv.stop()
            elif yellow_direction == "FORWARD":
                print("Moving FORWARD")
                agv.go_ahead(10)  # 전진 속도 조정
                time.sleep(1)
                #agv.stop()
        else:
